...
(() => {
  ...
  fetch(
    `/php/api/v0/get_invoice.php?id_invoice=${invoiceId}&include_images=base64&sig=${sig}`
  )
    .then((res) => {
    ...
    .catch((error) => {
      ...
      showError(
        "Не удалось загрузить данные счёта. Попробуйте позже или проверьте соединение."
      );
    });
})();
...
saveButton.addEventListener("click", () => {
  const fullHtml = `<!doctype html><html lang="ru">${
    document.head.outerHTML
  }<body>${document.getElementById("doc").outerHTML}</body></html>`;
  ...
  if (docData) {
    ...
    filename = sanitizeFilename(
      `${docData.seller.name}-${doc.title}-${date}-${number}.html`
    );
  }
